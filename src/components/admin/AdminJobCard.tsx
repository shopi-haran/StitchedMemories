    ...
    >
      ...
      <div className="pt-3 border-t border-[#1D231E]/10 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* If Custom Stitched and not pending quote, provide direct access to stitch tracker if not already shown in bar */}
          {isCustomStitched && !isInProduction && (
            <button
              type="button"
              onClick={() => onOpenStitchTracker(order)}
              className="px-4 py-2 bg-[#2D5A43] hover:bg-[#234734] text-white text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center gap-1.5 shadow-2xs"
            >
              <Scissors className="w-3.5 h-3.5 text-[#E06C38]" />
